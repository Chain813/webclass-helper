 ...
 * @returns
 */
function createUserScriptMetadata(formatter, metadata) {
	const contents = [];
	// 最长字段
	const maxLength = Object.keys(metadata).reduce((pre, cur) => Math.max(cur.length, pre), 0);

	const lineBuilder = (key, value) =>
 ...
 * @param require 外部依赖
 */
async function parseMetaDataRequires(require = []) {
	const requires = [];
 ...
 */
async function parseMetaDataResources(resource = [], resourceBuilder) {
	const resources = [];
 ...
 * 创建用户脚本
 */
async function createUserScript(opts) {
	let requires = '';
	let resources = '';
	...
	const entryContent = [
		// 可选的基础入口文件
		opts.baseEntry ? readFileSync(opts.baseEntry).toString() : '',
		// 入口文件
...
module.exports = {
	...
	parseMetaDataRequires,
	parseMetaDataResources,
	createUserScript,
	DEFAULT_METADATA
};
