...
const { createUserScript } = require('./script.builder');
...
async function buildPackages() {
	...
	const scriptsDir = path.resolve(__dirname, '../packages/scripts');
	const env = { ...process.env, VITE_BUILD_PATH: path.resolve(__dirname, distPath) };
	// @ts-ignore
	await execOut('npx tsc', { cwd: coreDir });
	// @ts-ignore
	await execOut('npx vite build', { cwd: coreDir, env });
	// @ts-ignore
	await execOut('npx tsc', { cwd: scriptsDir });
	// @ts-ignore
	await execOut('npx vite build', { cwd: scriptsDir, env });
}
...
async function createUserJs() {
	...
	const createOptions = () => {
		...
		return {
			...
			baseEntry: path.join(__dirname, '../packages/scripts/entry.base.js'),
	...
	devOpts.baseEntry = path.join(__dirname, '../packages/scripts/entry.base.js');
	...
	commonOpts.baseEntry = path.join(__dirname, '../packages/scripts/entry.base.js');
