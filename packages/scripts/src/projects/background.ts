import { RemotePlaywright, request, BACKEND_BASE_URL } from '@ocsjs/core';
		...
		appConfigSync: new Script({
			...
			async onactive() {
				if ($.isInTopWindow()) {
					...
					try {
						const res = await request(`${BACKEND_BASE_URL}/browser`, {
						...
						const open_sync = await request(`${BACKEND_BASE_URL}/is-browser-config-sync`, {
							type: 'GM_xmlhttpRequest',
							method: 'get',
							responseType: 'text'
						});
			...
			methods() {
				return {
					getLastVersion: async () => {
						return (await request(`${CDN_BASE_URL}/ocs-version.json?t=` + Date.now(), {
							method: 'get',
							type: 'GM_xmlhttpRequest'
