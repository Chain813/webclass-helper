...
import { CDN_BASE_URL } from '../utils/const';
import { trackedAnswerWrapperHandler } from '../utils/work';
...
/** 题库连续失败次数记录 */
type BankFailureRecord = Record<string, number>;

/** 题库连续失败自动禁用阈值 */
const MAX_BANK_FAILURES = 3;

/**
 * 创建 DeepSeek AI 题库配置
 */
function createDeepSeekWrapper(apiKey: string): AnswererWrapper {
	return {
		name: 'DeepSeek AI',
			...
			messages: [
				{
					role: 'user',
					content:
						'你是一个答题助手。请直接给出以下题目的答案，只返回答案内容，不要任何解释和多余文字。' +
						'如果是选择题，只返回选项字母（如 A 或 A,B,C）。' +
						'如果是判断题，只返回 正确 或 错误。' +
						'如果是填空题，只返回填空内容，多个空用换行分隔。' +
						'\n\n题目类型：${type}\n题目：${title}\n选项：${options}'
				}
			],
			temperature: 0.1,
	...
	scripts: {
		...
		settings: new Script({
			...
			configs: {
				...
				deepseekEnabled: {
					separator: 'AI 答题',
					label: '启用 DeepSeek AI',
					defaultValue: false,
					attrs: { type: 'checkbox', title: '启用 DeepSeek AI 作为辅助答题源，当题库搜不到答案时使用 AI 生成。' }
				},
				deepseekApiKey: {
					...
					attrs: {
						type: 'password',
						placeholder: '输入 DeepSeek API Key（sk-xxx）',
						title: '在 https://platform.deepseek.com 获取 API Key。密钥仅存储在本地浏览器中，不会上传到任何服务器。'
					}
				},
				bankFailureCounts: {
					defaultValue: {} as BankFailureRecord
				}
			},
			methods() {
				return {
					...
					getWorkOptions: () => {
						 ...
						 * 过滤掉连续失败超过阈值的题库
						 */
						const failureCounts = this.cfg.bankFailureCounts || {};
						workOptions.answererWrappers = workOptions.answererWrappers.filter((aw) => {
							const failures = failureCounts[aw.name] || 0;
							if (failures >= MAX_BANK_FAILURES) {
								console.warn(`题库 "${aw.name}" 连续失败 ${failures} 次，已自动跳过`);
								return false;
							}
							return true;
						});
						 ...
						 * 注入 DeepSeek AI 题库
						 */
						if (this.cfg.deepseekEnabled && this.cfg.deepseekApiKey?.trim()) {
							workOptions.answererWrappers.push(createDeepSeekWrapper(this.cfg.deepseekApiKey.trim()));
						}
					 ...
					 * 记录题库查询失败
					 */
					recordBankFailure: (bankName: string) => {
						const counts: BankFailureRecord = { ...(this.cfg.bankFailureCounts || {}) };
						counts[bankName] = (counts[bankName] || 0) + 1;
						this.cfg.bankFailureCounts = counts;

						if (counts[bankName] >= MAX_BANK_FAILURES) {
							$message.error({
								content: `题库 "${bankName}" 连续失败 ${counts[bankName]} 次，已自动禁用。请检查题库配置或网络连接。`,
								duration: 10
							});
							this.methods.notificationBySetting(`题库 "${bankName}" 已自动禁用`, {
								extraTitle: '题库状态',
								duration: 30
							});
						}
					 ...
					 * 记录题库查询成功（重置失败计数）
					 */
					recordBankSuccess: (bankName: string) => {
						const counts: BankFailureRecord = { ...(this.cfg.bankFailureCounts || {}) };
						if (counts[bankName]) {
							delete counts[bankName];
							this.cfg.bankFailureCounts = counts;
						}
					},
					/**
					 * 重置所有题库失败计数
					 */
					resetBankFailureCounts: () => {
						this.cfg.bankFailureCounts = {};
						$message.success({ content: '已重置所有题库状态', duration: 3 });
					},
					/**
					 * 根据全局设置的配置，发起通知
					 * @param content
			...
			onrender({ panel }) {
				...
				if ($gm.isInGMContext()) {
					...
					const resetFailures = h('button', { className: 'base-style-button' }, '🔄重置题库状态');
					resetFailures.title = '重置所有题库的失败计数，恢复被自动禁用的题库';
					resetFailures.onclick = () => {
						this.methods.resetBankFailureCounts();
						updateState();
					};
					...
					panel.body.append(
						h('div', { style: { display: 'flex', gap: '4px' } }, [testNotification, refresh, resetFailures]),
						tableContainer
					);
		...
		onlineSearch: new Script({
			...
			onrender({ panel }) {
				...
				const search = async (value: string) => {
					...
					if (value) {
						const t = Date.now();
						const infos = await trackedAnswerWrapperHandler(CommonProject.scripts.settings.cfg.answererWrappers, {
							title: value
						});
						// 耗时计算
			...
			methods() {
				return {
					...
					async showChangelog() {
						...
						const md = await request(`${CDN_BASE_URL}/articles/ocs/changelog.md?t=` + Date.now(), {
							type: 'GM_xmlhttpRequest',
							responseType: 'text',
							method: 'get'
						});
