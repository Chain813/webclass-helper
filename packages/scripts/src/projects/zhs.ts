...
import { MAX_RELOAD_COUNT } from '../utils/const';
...
const DEFAULT_EXCEEDED_MESSAGE =
	'视频加载失败/黑屏导致重新加载页面次数超过3次，请尝试关闭页面重新打开，或者检查网络连接！';

function createReloadHandler(opts: {
	reloadWhenError: boolean;
	onReload: () => void;
	onSkip: () => void;
	reloadDelay?: number;
	...
	return async (e: any) => {
		$console.error(e);
		if (opts.reloadWhenError) {
			const msg = '视频加载失败，即将刷新页面。';
			const reload_count = await $store.getTab('reload-count');
			if (reload_count && reload_count > MAX_RELOAD_COUNT) {
				const exceededMsg = opts.exceededMessage ?? DEFAULT_EXCEEDED_MESSAGE;
				await $store.setTab('reload-count', 0);
				$message.error({ content: exceededMsg, duration: 0 });
				$console.log(exceededMsg);
				CommonProject.scripts.settings.methods.notificationBySetting(exceededMsg, {
					duration: 0,
...
async function watch(
	...
	const reload = createReloadHandler({
		reloadWhenError: options.reloadWhenError,
		onReload: () => actions.reload(),
		onSkip: () => actions.onended({ next: true }),
		reloadDelay: 5000,
		exceededMessage: '视频加载失败导致重新加载页面次数超过3次，请尝试关闭页面重新打开，或者检查网络连接！'
	});
		...
		answerer: (elements, ctx) => {
			...
			if (title) {
				return CommonProject.scripts.apps.methods.searchAnswerInCaches(title, async () => {
					...
					return trackedAnswerWrapperHandler(answererWrappers, {
